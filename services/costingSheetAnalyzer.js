...
export function findCostingSheet(workbook, preferredName = "AUTO CALCULATION SHEET (2)") {
  ...
  for (const sheetName of workbook.SheetNames) {
    ...
    if (preferredNormalized && normalizedName) {
      if (normalizedName === preferredNormalized) {
        ...
        score += 80;
      } else if ((normalizedName.includes("calculation") || normalizedName.includes("auto")) && (preferredNormalized.includes("calculation") || preferredNormalized.includes("auto"))) {
        score += 40;
      }
    }
...
// In-memory cache keyed by docket number to avoid re-analyzing same workbook repeatedly
...
/**
 * Cached analyzer: returns cached analysis for a docket if present,
 * otherwise runs analysis and stores it in memory.
 * Keyed by docketNo to satisfy callers that identify work by docket.
 */
export function analyzeCostingSheetCached(filePath, docketNo) {
  try {
    if (!docketNo) {
