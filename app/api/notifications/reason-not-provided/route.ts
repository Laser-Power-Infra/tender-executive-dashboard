...
const NOTIFICATION_VALUE = NOTIFICATION_TYPES.DEADLINE_OVER_REASON_NOT_PROVIDED;
const RECIPIENT = process.env.EVOLUTION_REASON_NOT_PROVIDED_NOTIFICATION_NUMBER;

async function triggerReasonNotProvidedNotifications() {
  const matches = await prisma.tenderMerged.findMany({
    where: {
      AND: [
        { deadline: { gt: NOTIFICATION_DEADLINE_START, lte: new Date() } },
        { apm: "YES" },
        { OR: [{ participated: false }, { participated: null }] },
...
const triggerReasonNotProvidedWithLog = withLog(
  triggerReasonNotProvidedNotifications,
  (result) => ({
    action: "UPDATE",
    tableName: "TenderMerged",
    recordId: result.referenceNos.join(",") || undefined,
    details: `Triggered ${result.triggeredCount} reason-not-provided notifications (${result.failedCount} failed, ${result.total} matched)`,
  }),
);
...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = await triggerReasonNotProvidedWithLog();
    return NextResponse.json(result);
  } catch (error) {
    console.error("[notifications/reason-not-provided] Error:", error);
